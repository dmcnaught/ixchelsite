...
async function scrapeCurrentMonth(page) {
    ...
    const data = await page.evaluate((rooms) => {
        ...
        allDates.forEach(dateStr => {
            ...
            rooms.forEach(room => {
                result[room][day] = reservedDates[room][dateStr] || [];
            });
        });
...
async function main() {
    ...
    try {
            ...
            if (parsed) {
                ...
                for (const room of ROOMS) {
                    for (let day = 1; day <= daysInMonth; day++) {
                        ...
                        output.rooms[room][dateStr] = data[room]?.[day] || [];
                    }
                }
                const res2603 = Object.values(data['2603']).filter(v => Array.isArray(v) && v.length > 0).length;
                const res2604 = Object.values(data['2604']).filter(v => Array.isArray(v) && v.length > 0).length;
                console.log(`  ✓ ${monthLabel}: 603=${res2603} reserved days, 604=${res2604} reserved days`);
            } else {
        ...
        let emailBody = "Reservation Details:\n\n";
        for (const room of ROOMS) {
            let roomDetails = [];
            const dates = Object.keys(output.rooms[room]).sort();
            for (const date of dates) {
                const reservations = output.rooms[room][date];
                if (Array.isArray(reservations) && reservations.length > 0) {
                    const dateObj = new Date(date + 'T12:00:00');
                    const formattedDate = dateObj.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' });
                    reservations.forEach(res => {
                        roomDetails.push(`• ${formattedDate}: ${res}`);
                    });
                }
            }
